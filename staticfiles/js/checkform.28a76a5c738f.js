...
// const idPUrl = document.getElementById('id_p_url');
// let checkForm = document.getElementById('checkform');

if (heatFactor) {
    heatFactor.onclick = function(e) {

        let idPUrl = document.getElementById('id_p_url');
        let checkForm = document.getElementById('checkform');

        if (!psRegEx.test(idPUrl.value)) {
            e.preventDefault();
            checkForm.style.display = "block";
            checkForm.innerHTML = psBadUrlMsg;
            checkForm.classList.toggle('fade');
        } else {
            displaySpinner();
        }
    }
