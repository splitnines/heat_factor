...
from .forms import (PractiscoreUrlForm, GetUppedForm, AccuStatsForm1,
                    AccuStatsForm2)
...
def home(request):
    ...
    if request.method == 'POST':
        ...
        if (practiscore_url_form.is_valid() and get_upped_form.is_valid() and
...
def heat_factor(request):
    ...
    ps_regex = (re.compile(r'^https://(www\.)?practiscore\.com/results/new/'
                           r'[0-9a-z-]+$'))
    if re.search(ps_regex, url):
        ...
        if 'match_name' in match_def:
            ...
            return render(request, 'heat_factor.html',
...
def get_upped(request):
    ...
    try:
        ...
        return render(request, 'get_upped.html', {'response_text':
                '<font color=\"red\">2 Mikes, 2 No-shoots:</font> No scores '
                f'found for memeber <font color=\"red\">{mem_num}</font> in '
                f'<font color=\"red\">{division}</font> division.  If your '
                'USPSA classifier scores are set to priviate this tool '
    ...
    if shooter.get_shooter_class() == 'U':
        return render(request, 'get_upped.html', {'response_text':
                ...
                f'{str(shooter.get_initial_classifaction()[0])}%</font> in '
                'your next classifier to achieve an initial classification '
                'of <font color=\"green\">'
    ...
    if shooter.get_upped() > 100:
        return render(request, 'get_upped.html', {'response_text':
                ...
                'need a score greater than <font color=\"red\">100%</font>. '
                f'Enjoy {shooter.get_shooter_class()} class',
                'date': dt.datetime.now()})
    else:
        return render(request, 'get_upped.html', {'response_text':
                'You need a score of <font color=\"green\">'
                f'{str(shooter.get_upped())}%</font> to make '
                f'<font color=\"green\">{shooter.get_next_class()}</font> '
                'class.', 'date': dt.datetime.now()})
...
def points(request):
    ...
    mem_num            = request.POST.get('mem_num')
    delete_match       = (request.POST.get('delete_match')
                         if type(request.POST.get('delete_match')) ==
                         str else '')
    ...
    try:
        (scores_df,
         shooter_fn,
         shooter_ln) = create_dataframe(match_links_json, match_date_range,
                                        delete_list, mem_num)
    except:
